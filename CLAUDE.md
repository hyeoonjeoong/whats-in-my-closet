...
### 진행 중인 Plan 확인
작업 시작 전 `.claude/plans/` 폴더에 진행 중인 plan이 있는지 확인합니다.

### 체크리스트 업데이트
기능 구현 완료 시 해당 plan 문서의 체크리스트를 업데이트합니다:
- `[ ]` → `[x]` 로 변경
- 완료된 항목 아래에 간단한 메모 추가 가능

### 완료 알림
체크리스트 항목을 완료할 때마다 사용자에게 진행 상황을 알립니다:
```
✅ 체크리스트 업데이트
   [x] 완료된 항목
   [ ] 남은 항목 1
...
### 전체 완료 시
모든 체크리스트가 완료되면 사용자에게 알리고 `/task-done` 실행을 제안합니다:
```
🎉 모든 체크리스트 완료!
   `/task-done [기능명]` 으로 완료 처리하시겠어요?
```

### Plan 문서 위치
- 진행 중: `.claude/plans/YYYY-MM-DD-기능명.md`
- 완료: `.claude/plans/완료/YYYY-MM-DD-기능명.md`
